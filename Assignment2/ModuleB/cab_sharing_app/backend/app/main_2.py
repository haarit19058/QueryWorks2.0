...
DATABASE_URL = "mysql+mysqlconnector://root:RK056@172.27.236.113:3306/RideShare"
...
# Updated Code
    ...
    # fetch all passengers in one go
...
def get_ride_history(db: Session = Depends(get_db), user: Member = Depends(get_current_user)):
    ...
    # --- main queries (same as before) ---
    hosted = db.query(RideHistory).filter(
    ...
    all_rides = hosted + taken
    ride_ids = [r.RideID for r in all_rides]

    # --- 🔥 preload everything (only 3 queries total) ---

    # 1. feedback
    feedback_set = {
        f.RideID
        for f in db.query(RideFeedback)
        .filter(
            RideFeedback.MemberID == user.MemberID,
            RideFeedback.RideID.in_(ride_ids)
        )
    }

    # 2. admins
    admin_ids = list(set(r.AdminID for r in all_rides))
    admins = (
        db.query(Member.MemberID, Member.FullName)
        .filter(Member.MemberID.in_(admin_ids))
        .all()
    )
    admin_map = {a.MemberID: a.FullName for a in admins}

    # 3. passengers
    passenger_data = (
        db.query(RidePassengerMap.RideID, Member.MemberID, Member.FullName)
        .join(Member, Member.MemberID == RidePassengerMap.PassengerID)
        .filter(RidePassengerMap.RideID.in_(ride_ids))
        .all()
    )
    ...
    # --- optimized serialize (no DB calls) ---
    def serialize(ride: RideHistory, role: str):
        return {
            **{c.name: getattr(ride, c.name) for c in ride.__table__.columns},
            "Role": role,
            "Passengers": passenger_map.get(ride.RideID, []),
            "AdminName": admin_map.get(ride.AdminID, "Unknown"),
            "HasFeedback": ride.RideID in feedback_set,
        }
