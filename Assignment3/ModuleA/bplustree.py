...
class BPlusTree:
    ...
    def apply_redo_op(self, payload):
        """
        Redo an operation from WAL.
        payload: { 'table': table_name, 'key': key, 'op': 'INSERT'|'UPDATE'|'DELETE',
                   'old': ..., 'new': ... }
        """
        op = payload.get("op")
        key = payload.get("key")
    ...
    def apply_undo_op(self, payload):
        """
        Undo an operation from WAL.
        payload: { 'table': table_name, 'key': key, 'op': 'INSERT'|'UPDATE'|'DELETE',
                   'old': ..., 'new': ... }
        """
        op = payload.get("op")
    ...
    def save_to_disk(self, filepath):
        """
        Serialize the B+ tree structure and save to disk.
        """
    ...
    def load_from_disk(filepath):
        """
        Load a B+ tree structure from disk.
        """
