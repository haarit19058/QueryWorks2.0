...
class DatabaseManager:
    ...
    def insert(self, table_name, row):
        """
        Commits a new record to the persistent index first, then updates memory.
        Ensures atomicity if a disk/serialization error occurs.
        """
        table = self.tables[table_name]
        tree = self.indexes[table_name]
        ...
        # 1. Do the risky disk operation FIRST
        try:
            tree.insert(pk_value, row)
        except Exception as e:
            # If the disk write or serialization fails, stop here!
            # We raise the error so the user knows it failed, and memory is never touched.
            raise RuntimeError(f"Database insert failed on disk: {e}")

        # 2. If (and only if) the disk succeeds, commit to the in-memory table
        table.insert(row)
    ...
    def delete(self, table_name, key):
        ...
        try:
            ...
            raise RuntimeError(f"Database delete failed on disk: {e}")

        # 2. Delete from memory only if disk succeeds
